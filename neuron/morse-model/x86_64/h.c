...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 1, "cvodeieq");
 	...
 	ivoc_help("help ?1 h /home/alec/code/xolotl-paper/neuron/morse-model/x86_64/h.mod\n");
...
double taum (  double _lVm ) {
   double _ltaum;
  _ltaum = 272.0 + 1499.0 / ( 1.0 + exp ( - ( _lVm + 42.2 ) / 8.73 ) ) ;
...
static int  rates (  double _lVm ) {
   tau_m = taum ( _threadargscomma_ _lVm ) ;
    minf = 1.0 / ( 1.0 + exp ( ( _lVm + 70.0 ) / 6.0 ) ) ;
     return 0; }
...
{
 { error =  states();
 if(error){fprintf(stderr,"at line 36 in file h.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }}}
