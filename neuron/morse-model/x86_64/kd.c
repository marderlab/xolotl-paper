...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 1, "cvodeieq");
 	...
 	ivoc_help("help ?1 kd /home/alec/code/xolotl-paper/neuron/morse-model/x86_64/kd.mod\n");
...
double taum (  double _lVm ) {
   double _ltaum;
  _ltaum = 7.2 - 6.4 / ( 1.0 + exp ( - ( _lVm + 28.3 ) / 19.2 ) ) ;
...
static int  rates (  double _lVm ) {
   tau_m = taum ( _threadargscomma_ _lVm ) ;
    minf = 1.0 / ( 1.0 + exp ( - ( _lVm + 12.3 ) / 11.8 ) ) ;
     return 0; }
...
{
 { error =  states();
 if(error){fprintf(stderr,"at line 36 in file kd.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }}}
