...
#define Dh _p[10]
#define _g _p[11]
#define gbar	*_ppvar[0]._pval
#define _p_gbar	_ppvar[0]._pval
...
#define tauh tauh_ka
#define taum taum_ka
 extern double tauh( double );
 extern double taum( double );
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 1, "cvodeieq");
 	...
 	ivoc_help("help ?1 ka /home/alec/code/xolotl-paper/neuron/morse-model/x86_64/ka.mod\n");
...
static int  rates (  double _lVm ) {
   ...
   tau_m = taum ( _threadargscomma_ _lVm ) ;
    minf = 1.0 / ( 1.0 + exp ( - ( _lVm + 27.2 ) / 8.7 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lVm + 56.9 ) / 4.9 ) ) ;
     return 0; }
...
{
 { error =  states();
 if(error){fprintf(stderr,"at line 38 in file ka.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }}}
