...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 0, "cvodeieq");
 	...
 	ivoc_help("help ?1 na /home/marder/code/simulation-environment-paper/neuron/x86_64/na.mod\n");
...
double taum ( _threadargsprotocomma_ double _lVm ) {
   double _ltaum;
  _ltaum = 1.32 - 1.26 / ( 1.0 + exp ( ( _lVm + 120.0 ) / ( - 25.0 ) ) ) ;
...
double tauh ( _threadargsprotocomma_ double _lVm ) {
   double _ltauh;
  _ltauh = ( 0.67 / ( 1.0 + exp ( ( _lVm + 62.9 ) / ( - 10.0 ) ) ) ) * ( 1.5 + 1.0 / ( 1.0 + exp ( ( _lVm + 34.9 ) / 3.6 ) ) ) ;
...
static int  rates ( _threadargsprotocomma_ double _lVm ) {
    minf = 1.0 / ( 1.0 + exp ( ( _lVm + 25.5 ) / ( - 5.29 ) ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lVm + 48.9 ) / 5.18 ) ) ;
