...
Our goal in designing "xolotl" was to create an easy-to-use neuron and network simulator that was fast enough and accurate enough for routine use by computational neuroscientists. In this section, we compare the speed of "xolotl" in simulating single and large numbers of Hodgkin-Huxley-like neuron models (as in Figure \ref{fig:figurehh}) and bursting neuron models based on the bursting neurons in the lobster STG (\cite{prinzSimilarNetworkActivity2004}). For each type of neuron model, we compared our software to  "NEURON" (\cite{hinesNEURONSimulationEnvironment1997}), a high-performance and powerful neuron simulator specialised in simulating neurons with complex morphologies and "DynaSim" (\cite{sherfeyDynaSimMATLABToolbox2018}), a general-purpose simulator that can solve coupled differential equations numerically. All simulators were run on the same hardware using fixed time step solvers: "xolotl" used the Exponential Euler method  (\cite{dayanTheoreticalNeuroscience2001}), "NEURON" used the  implicit Euler solver (\cite{hinesNEURONSimulationEnvironment1997}) and "DynaSim" used "C"-compiled 2\textsuperscript{nd}-order Runge-Kutta integration scheme as recommended for high-performance in the documentation. We measured the speed of each simulator by dividing the time simulated for by the time it took for the simulator to complete. For example, if a simulator could simulate 10 seconds of neuron data in 1 second, its speed would be 10X. We measured the speed of every simulator as a function of simulation time step, total length of simulation, and system size. 

All three simulators were faster with larger time steps, since fewer iterations were needed (Figure \ref{fig:figurebenchmark}A-B), and were approximately linear in the region tested. Our simulator compared favourably to "NEURON" and "DynaSim" in this task. We also measured the quality of the simulated output by comparing it to the simulated output at the smallest time step. Simulation error was measured using the LeMasson cost (\cite{lemassonIntroductionEquationSolving2000}), and was comparable amongst the three simulators (Figure \ref{fig:figurebenchmark}C-D). Our simulator sets up and runs the simulation in "C++", and needs to transfer parameters and data to and from the underlying implementation. To measure the performance cost of this overhead, we repeated these simulations on all three simulators at a afixed time step of $0.1$ ms and varied the length of time simulated for. Speed increased with simulation duration up to a limit, and then saturated, indicating a fixed performance cost of the overhead (Figure \ref{fig:figurebenchmark}E-F, black lines). Simualtions using "DynaSim", which also used a similar architecture, showed a similar increase in speed with simulation length (Figure \ref{fig:figurebenchmark}E-F, red lines). In contrast, simulations using "NEURON" ran at a constant speed irrespective of simulation length, presumably due to differences in the underlying implementation (Figure \ref{fig:figurebenchmark}E-F, blue lines).    

Many simulators have been designed to simualte large numbers of compartments, either as networks with many identical neurons or in a large mulit-compartmentel neuron model (\cite{bretteSimulationNetworksSpiking2007, sherfeyDynaSimMATLABToolbox2018, vitayANNarchyCodeGeneration2015, delormeSpikeNETEventdrivenSimulation2003}). While our software is not designed for this task {\em per se}, we measured its performance as a function of the number of compartments simulated. "xolotl" can quickly create a number of identical copies of a compartment using the "replicate" method: 

\begin{lstlisting}[style=Matlab-editor]
x.replicate('compartment_to_replicate',n_copies); 
\end{lstlisting}

We used the "replicate" method to create simulations with varying numbers of neurons (either Hodgkin-Huxley-like or bursting neurons) and measured the speed of all three simulators as a function of system size. Plotting the speed normalised by the system size, we observed that the speed of integration of "xolotl" is linear with system size (Figure \ref{fig:figurebenchmark}G-H, black lines), for up to 1000 single-compartment neurons (up to 13,000 ODEs). Its performance compares favourably with that of "NEURON" and "DynaSim" as a function of system size (Figure \ref{fig:figurebenchmark}G-H).
...
The authors would like to thank Timothy O'Leary and Leandro Alonso for useful discussions. Janis Li helped to prepare many conductance header files.
